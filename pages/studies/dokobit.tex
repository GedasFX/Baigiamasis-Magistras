...
On Apr 11, 2022, Dokobit developers fixed the issue by blocking all subsequent requests to the {authorize} endpoint. This fix prevents attackers from abusing the automatic redirect feature. It would leave us with an attack vector where an attacker would target the callback endpoint. Because Dokobit allows customizing redirect URLs, creating an opaque challenge passed in the query parameters is possible, eliminating this exploit. 

Figures \ref{fig:dokobit-vuln-csrf-fix-a} and \ref{fig:dokobit-vuln-csrf-fix-b} illustrate how the page looks after the fix was deployed. A page reload is treated as a subsequent visit. As an unwanted side-effect, if a user accidentally reloads the page, they would not be able to authenticate and would need to create a new session.
