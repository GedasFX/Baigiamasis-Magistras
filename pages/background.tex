...
% \subsection{Difference between Authentication and Digital Signature}

% The requirements for Estonian ID cards make a clear distinction between "ADF AWP" and "ADF QSCD" applications \cite{ee-id-tech}. Both of these applications are intended to perform cryptographic operations. However, the purposes of each application are different. The AWP module handles authentication, encryption, and other processes, whereas the QSCD is used exclusively for signing documents. This separation means at least some level of distinction between authentication and signing.

% Under eIDAS there are a lot stricter requirements 
