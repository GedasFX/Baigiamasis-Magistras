...
For the authorize URL generation is best to look at chapter 4.1 of TARA documentation \cite{tara-technical}. There are, however, some notes to keep in mind when integrating the eeID service:
...
\begin{itemize}
  ...
  \item {nonce, state}: when using nonce, it is theoretically possible to not use the state property, however the value of nonce is copied over to the identity token, and therefore developers should avoid making a long nonce value. State should be used when transfering information.
\end{itemize}
